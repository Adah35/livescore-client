...
import { useGetCountryListQuery, useGetLeaguesByCountryQuery } from "./livescoreApislice";
import { Link } from "react-router-dom";
import { RiArrowGoBackLine, RiArrowLeftLine, RiArrowLeftSLine, RiArrowRightSLine } from "react-icons/ri";
import { ImSad, ImSpinner } from 'react-icons/im'

const CountryLeagues = ({ country_id }) => {
  const { data, isLoading, isSuccess, isError, error } = useGetLeaguesByCountryQuery(country_id)
  let content
  if (isSuccess) {
    data.map((v, i) => {
      return (
        content = (<Link >
          <li className="mb-3.5">
            ...
            >
              <img className="w-6 h-5" src={v.league_logo} alt={v.league_name} />
  ...
  return (
    <li className="mb-3.5">
      <Link><span
        className="text-[#181818] font-semibold text-sm flex flex-row items-center gap-2"
      >
        <img className="w-6 h-5" src={data.country_logo} alt={data.country_name} />
...
const CountryList = () => {
  const { data: countryData, isLoading, isSuccess, isError, error } = useGetCountryListQuery(undefined, {
  ...
  if (isError) {
    content = (
      <div className=''>
        ...
        <p className='text-xl text-center text-gray-400'>{error?.data?.message || 'Sorry no data found'}</p>
      </div>
    )
  }

  if (isSuccess) {
    content = countryData.map((v, i) => {
      return (
    ...
    <aside className="w-full">
      {!selectedCountryId ?
        <div class="relative flex items-center border border-gray-300 rounded px-3 py-2 bg-transparent">
          ...
          /> </div> : <p className="text-[#181818] flex items-center gap-1 text-lg cursor-pointer hover:text-blue-600 font-semibold" onClick={() => setSelectedCountryId(null)}><RiArrowLeftSLine /> country</p>}
      <ul>
        {selectedCountryId ?
          <CountryLeagues country_id={selectedCountryId} /> // Display the leagues for the selected country
          :
          content // Display the list of countries
        }</ul>
    </aside>
  );
};
