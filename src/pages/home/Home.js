    ...
    if (isError) {
        content = (
            <div className=''>
                ...
                <p className='text-2xl text-center text-gray-400'>{error?.data?.message || 'Sorry no data found'}</p>
            </div>
        )
    }

    if (isSuccess) {
        content = livescoreData.map((v, i) => {
            const url = `${v.country_name}/${v.league_name?.replace('%', '-')}/${v.match_hometeam_name}vs${v.match_awayteam_name}/${v.match_id}/match`
            return (
                <Link to={url}><Livescore data={v} /></Link>
            )
        })
    }
